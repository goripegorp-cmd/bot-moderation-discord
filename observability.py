...
# (Les helpers `_anomaly_dm_already_sent`, `_resolve_owner_member` et
#  `_dm_owner_anomaly` ont été supprimés avec le MP qu'ils servaient. Le bloc envoyait
#  par DEUX chemins — `dm_digest.send_urgent_now` puis `owner.send` en repli : couper la
#  seule ligne `owner.send` n'aurait rien coupé du tout.
#  `daily_snapshot_task`, qui poste en SALON, n'est pas touchée.)
        ...
        for guild in list(_bot.guilds):
            try:
                ...
                for a in anomalies:
                    ...
                    # C'était le JUMEAU de l'alerte « perte nette de membres » de bot.py :
                    # même sujet (signal « 📉 Départs de membres »), autre tuyau, autre
                    # cadence. Supprimer l'un sans l'autre n'aurait rien changé pour lui.
                    # L'anomalie reste DÉTECTÉE et journalisée juste en dessous, et reste
                    # consultable dans le panneau des anomalies.
                    await log_anomaly(
