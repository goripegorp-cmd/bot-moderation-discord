...
| `verif_boutons_persistants.py` | Tout `custom_id` posé sur une `View`/`LayoutView` en `timeout=None` doit être capté par un `bot.add_view` ou un `add_dynamic_items`. Sinon le bouton s'affiche, se clique, et Discord répond « n'a pas répondu à temps » — en public. **Prend plusieurs fichiers** (`bot.py roblox_panneau.py`) : un bouton peut être posé dans un module et enregistré dans un autre. Exempte les boutons dont le `callback` est attaché en direct (panneaux éphémères). |
