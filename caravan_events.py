...
_CLICK_CD = 3.0
_REFRESH_MIN = 4.0
_SPAWN_COOLDOWN_H = 4.0       # Phase 267 : espacé (anti-bruit) 2.0→4.0
_SPAWN_CHANCE = 0.12          # Phase 267 : abaissé 0.22→0.12 (moins de spawns collab)
_BASE_REWARD = 80
_PER_STAGE = 25
_COLOR = 0xE67E22
...
async def spawn_caravan(guild: discord.Guild) -> bool:
    ...
    try:
        ...
        # FIX salons (anti-salon-vide) : panneau non posté → clore la caravane (sinon
        # ended=0 + ends_at futur bloque _has_any_major_event_running et le sweeper) +
        # supprimer le salon « 🐫 » désormais vide. Réutilise les helpers de _end_caravan.
        # Fail-open.
        try:
            if 'cid' in locals() and cid:
                async with _get_db() as db:
                    await db.execute(
                        "UPDATE caravan_events SET ended=1 WHERE id=? AND ended=0",
                        (cid,))
                    await db.commit()
        except Exception:
            pass
