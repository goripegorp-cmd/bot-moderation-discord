...
# ── MENACES / INTIMIDATION IRL (owner 2026-06-27) ────────────────────────────────────────
# Menacer quelqu'un de violence réelle ou de le retrouver IRL = GRAVE (parfois illégal). On
# EXCLUT VOLONTAIREMENT le banter gaming (« je vais te tuer / te défoncer / te one-shot / i'm
...
def threat_hit(raw: str, normalized: str | None = None):
    """Renvoie la MENACE IRL trouvée (violence réelle / « je sais où t'habites » / intimidation),
    ou None. Banter gaming EXCLU du lexique. Scan brut + plié + normalisé. FAIL-SAFE → None."""
    try:
        if not raw:
            return None
        variants = [raw]
